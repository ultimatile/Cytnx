...
def tdvp1_XXZmodel_dense(J, Jz, hx, hz, A, chi, dt, time_step):
    ...
    class ZeroSiteOp(cytnx.LinOp):
        def __init__(self, L, R):
            ...
            self.anet.FromString(["C: -1,-2", "L: -3,-1,0", "R: -3,-2,1", "TOUT: 0;1"])
    ...
    for k in range(1, time_step + 1):
        ...
        for p in range(Nsites - 1, -1, -1):
            ...
            anet.PutUniTensors(
                ["R", "B", "M", "B_Conj"], [LR[p + 1], A[p], M, A[p].Conj()]
            )
            old_LR = LR[p].clone()
            if p != 0:
                LR[p] = anet.Launch()
...
def Local_meas(A, B, Op, site):
    ...
    anet = cytnx.Network()
    anet.FromString(["l: 0,3", "A: 0,1,2", "B: 3,1,4", "TOUT: 2;4"])
    for i in range(0, N):
        if i != site:
...
def prepare_rand_init_MPS(Nsites, chi, d):
    ...
    A[0] = cytnx.UniTensor(
        cytnx.random.normal([1, d, min(chi, d)], 0.0, 1.0, seed=0), rowrank=2
    )
    A[0].relabels_(["0", "1", "2"])
    lbls.append(["0", "1", "2"])  # store the labels for later convenience.
    ...
    for k in range(1, Nsites):
        ...
        A[k] = cytnx.UniTensor(
            cytnx.random.normal([dim1, dim2, dim3], 0.0, 1.0, seed=0), rowrank=2
        )
